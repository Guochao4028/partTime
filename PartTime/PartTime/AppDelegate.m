...
//    self.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    ...
    NSArray *images = @[@"引导页1.jpg", @"引导页2.jpg"];
    BOOL isFirst = [XTGuidePagesViewController isShow];
    if (isFirst) {
...
-(void)pushInfoMainProgram
{
        RootViewController *vc = [[RootViewController alloc] init];
        self.window.rootViewController = vc;
}

-(void)pushInfoLoginVC
{
...
- (void)clickEnter
{
    NSString * string= [[NSUserDefaults standardUserDefaults] objectForKey:@"LoginSkip"];
    if (string !=nil) {
        if ([string isEqualToString:@"LoginSkip"]) {
            //            UserInfoModel * userName =[LoginManager getLoginUserInfo];
            [self pushInfoMainProgram];
        }else{
            // 设置根控制器
            [self pushInfoLoginVC];
        }
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"PartTime"];
